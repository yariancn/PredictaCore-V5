...
async function ejecutarProceso(jobId) {
    const input = jobs[jobId].dna.trim();
    let dossier = "";
    ...
    if (input.startsWith('http') || (input.includes('.') && !input.includes(' '))) {
        ...
        dossier = `IDEA DE NEGOCIO / CONCEPTO: ${input}`;
...
app.get('/download', async (req, res) => {
    ...
    if (!job || job.status !== 'done') return res.status(404).send("Reporte no listo");

    let htmlFinal = `<html><head>${ESTILOS_PDF}</head><body>`;
    htmlFinal += `<div class="logo-header">PREDICTACORE TITÁN</div><p>REPORTE FORENSE DE CONVERSIÓN</p>`;
    ...
    Object.keys(job.progress).forEach(etapa => {
        ...
        htmlFinal += `<section class="report-section">${texto}</section>`;
    });
    htmlFinal += `</body></html>`;

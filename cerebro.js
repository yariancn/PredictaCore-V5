...
const PROMPTS = {
  INTRO: (d) => `${IDIOMA}\n${REGLA_NUCLEAR}\n${PILARES_11}\nEscribe este encabezado traducido: ### I. INTRODUCCIÓN, RESUMEN EJECUTIVO Y RADIOGRAFÍA\nPARTE A — RESUMEN EJECUTIVO (OBLIGATORIO PRIMERO): Tabla Markdown con filas: Tiempo de carga (usa TIEMPO_CARGA_SEG del dossier), SEO técnico (usa SEO_TECNICO_SCORE), Visibilidad IA (usa AI_DISCOVERABILITY_SCORE), Veredicto forense (1 frase letal). Debajo, 3 bullets de hallazgos críticos sin repetir secciones posteriores.\nPARTE B — Identidad PredictaCore: gemelos sintéticos + evaluaciones medibles del bloque SIMULATION_RESULTS (cita EVALUACIONES_EJECUTADAS). No inventes números distintos al dossier.\nPARTE C — Radiografía del negocio (2 párrafos): qué venden, mercado, modelo. PROHIBIDO diagnosticar aquí. Dossier: ${d}`,

  GEMELOS: (d) => `${IDIOMA}\n${REGLA_NUCLEAR}\nEscribe este encabezado traducido: ### II. PERFILES PSICOLÓGICOS (GEMELOS SINTÉTICOS)\n4 perfiles alineados a las personas en SIMULATION_RESULTS (escéptico, apurado, mobile, comparador). EXACTAMENTE 2 oraciones por perfil: motivación + cómo reaccionarían a las FALLAS detectadas en su simulación. PROHIBIDO perfiles genéricos sin enlace a evaluaciones #id. Dossier: ${d}`,

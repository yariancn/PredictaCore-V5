...
const PROMPTS = {
  ...
  FUGAS: (d) => `${IDIOMA}\n${REGLA_NUCLEAR}\nEscribe este encabezado traducido al idioma detectado: ### VIII. 15 PUNTOS DE FUGA\nIdentifica 15 hallazgos críticos de fricción. Marca las peores como **[HEMORRAGIA CRÍTICA]**. REGLAS:\n1. ${FORMATO_LISTAS} (Numera del 1 al 15).\n2. Evalúa ERRORES MICRO (botones, usabilidad, contraste, carga) basándote en las imágenes y el dossier. Explica el impacto financiero de cada fuga.\n3. NODO DE CIERRE INTELIGENTE (OBLIGATORIO): Analiza el contexto del negocio antes de juzgar los botones de acción (Agregar al Carrito, Comprar, Reservar). REGLA FORENSE: Si el activo vende productos personalizables o depende de vender complementos (venta cruzada), la existencia de botones de pago rápido (Shop Pay, Apple Pay, GPay) en la página del producto es una HEMORRAGIA CRÍTICA, ya que incitan al cliente a saltarse el carrito y destruyen el Ticket Promedio. Si es un producto genérico de compra impulsiva, evalúa solo si hay choque o confusión visual entre los botones. Si es un servicio, evalúa si la ruta hacia la reserva es clara o está llena de fricción. Dossier: ${d}`,

...
const REGLA_TONO = `REGLA DE TONO (OBLIGATORIA FUERA DE SECCIÓN VIII FUGAS):
- Reserva **[Critical]** / **[Crítico]** EXCLUSIVAMENTE para los puntos 1–3 de la sección VIII (Fugas).
- En Intro, Scorecard, Visibilidad, Acciones, SWOT, etc.: usa **Priority**, **High impact**, **Needs attention**, **Elevated** (ES: **Prioritario**, **Alto impacto**, **Requiere atención**).
- PROHIBIDO: critical failure, catastrophic, hemorrhage, crítico, desastre — salvo en fugas 1–3.`;

const PRIORIDAD_FUGAS = `PRIORIDAD DE FUGAS (15 puntos — SOLO en sección VIII):
- Puntos 1-3: **[Critical]** (ES: **[Crítico]**) — bloquean conversión hoy.
- Puntos 4-8: **[High]** (ES: **[Alto]**) — abandono probable.
- Puntos 9-12: **[Medium]** (ES: **[Medio]**) — corregir en 30 días.
- Puntos 13-15: **[Low]** (ES: **[Bajo]**) — pulido.
Formato: 1. **[Critical]** Descripción. (evidence: evaluation #id)`;
...
const PROMPTS = {
  INTRO: (d) => `${IDIOMA}\n${REGLA_NUCLEAR}\n${REGLA_TONO}\n${PILARES_11}\nEscribe este encabezado traducido: ### I. INTRODUCCIÓN, RESUMEN EJECUTIVO Y RADIOGRAFÍA\nPARTE A — RESUMEN EJECUTIVO (OBLIGATORIO PRIMERO): Tabla Markdown con filas: Tiempo de carga (TIEMPO_CARGA_SEG), SEO técnico (SEO_TECNICO_SCORE), Visibilidad IA (AI_DISCOVERABILITY_SCORE), Giro detectado (GIRO_DETECTADO), Veredicto (1 frase: por qué pierde visitantes). Luego 3 puntos numerados (1. 2. 3.) de hallazgos **prioritarios** — PROHIBIDO "crítico/critical" aquí.\nPARTE B — Para el emprendedor (1 párrafo): auditamos SOLO lo público; objetivo = dejar de perder visitantes. PROHIBIDO cantidad de simulaciones.\nPARTE C — Radiografía del negocio (2 párrafos): qué venden, a quién, modelo. PROHIBIDO diagnosticar aquí. Dossier: ${d}`,
  ...
  SCORECARD: (d) => `${IDIOMA}\n${REGLA_NUCLEAR}\n${REGLA_TONO}\nEscribe este encabezado traducido: ### III. SCORECARD DE SALUD COMERCIAL\nTabla Markdown: | Punto de Salud | Calificación (1-10) | Diagnóstico |\n10 filas. Incluye SEO Técnico, Visibilidad IAs, Velocidad. Diagnósticos concisos — PROHIBIDO "crítico/critical" (usa "requiere atención", "prioritario"). Dossier: ${d}`,

  VISIBILIDAD: (d) => `${IDIOMA}\n${REGLA_TONO}\nEscribe este encabezado traducido: ### IV. VISIBILIDAD, SEO FORENSE Y DESCUBRIMIENTO POR IA\nUsa SEO_FORENSICS, AI_VISIBILITY, KEYWORDS_INFERIDAS.\n\nSUBSECCIÓN A — Tabla: | Señal | Valor | Estado (OK / Attention / Priority) | Impacto |. ES: OK / Atención / Prioritario. PROHIBIDO "Crítico/Critical" en esta tabla.\n\nSUBSECCIÓN B — Keywords inferidas (KW_*). SUBSECCIÓN C — Tabla IAs. SUBSECCIÓN D — Veredicto GEO/AIO (1 párrafo). Dossier: ${d}`,

  BENCHMARK: (d) => `${IDIOMA}\n${REGLA_NUCLEAR}\nEscribe este encabezado traducido: ### V. BENCHMARK COMPETITIVO\nOBLIGATORIO: Lee BENCHMARK_VERIFIED, UBICACION_MERCADO (CRITERIO_MERCADO) y PERFIL_NEGOCIO.\n- CRITERIO_MERCADO ZONA_ENVIO (e-commerce): competidores = mismo nicho que venden/envían al MERCADO_COMPETENCIA — la dirección física del negocio NO importa.\n- CRITERIO_MERCADO PRESENCIA_LOCAL (clínica, consultorio, restaurante, inmobiliario): competidores SOLO en CIUDAD_SERVICIO — PROHIBIDO otras ciudades.\n- MERCADO_NACIONAL: mismo nicho + mismo PAIS_MERCADO.\n- Si ESCALA=PYME_BOUTIQUE: PROHIBIDO Amazon, Walmart, Target, Costco u otros mega-retailers.\n- Si ESTADO=SIN_COMPETENCIA_IDENTIFICADA: NO uses tabla. Escribe 2-3 párrafos con MENSAJE_OBLIGATORIO.\n- Si COMP_1..3 existen: Tabla SOLO con esos dominios:\n| Criterio | Activo analizado | [COMP_1] | [COMP_2 si existe] | [COMP_3 si existe] |\n| **Qué hacen / What they do** | (1 frase) | (QUE_HACE COMP_1) | (COMP_2) | (COMP_3) |\n| Claridad de oferta | ... | ... | ... | ... |\n| Señales de confianza | ... | ... | ... | ... |\n| Visibilidad en IAs | ... | ... | ... | ... |\nPROHIBIDO dominios fuera de BENCHMARK_VERIFIED. Dossier: ${d}`,

  SWOT: (d) => `${IDIOMA}\n${REGLA_NUCLEAR}\nEscribe este encabezado traducido: ### VI. MATRIZ ESTRATÉGICA (ASISTIDA POR IA)\nUsa subsecciones con encabezados #### Fortalezas / #### Debilidades / #### Oportunidades / #### Amenazas (traducidos al idioma del activo).\nEn cada subsección: EXACTAMENTE 3-5 puntos numerados (1. 2. 3.) — PROHIBIDO párrafos corridos sin numeración. Basada en dossier + SIMULATION_RESULTS + COMP_* si existen. PROHIBIDO citar Amazon/Walmart/Target. ${FORMATO_LISTAS} Dossier: ${d}`,
  ...
  ACCIONES: (d) => `${IDIOMA}\n${REGLA_NUCLEAR}\n${REGLA_TONO}\nEscribe este encabezado traducido: ### IX. 15 ACCIONES TÁCTICAS\n15 líneas numeradas. Acción N resuelve fuga N. Formato: 1. **[Título breve]:** pasos. ${FORMATO_LISTAS}. **[COPIAR Y PEGAR]:** cuando aplique.\nAcción 1 o 2: **Visibilidad en IAs (GEO/AIO)** según AI_VISIBILITY. PROHIBIDO "critical" en títulos. Dossier: ${d}`,
...
module.exports = { PROMPTS, IDIOMA, REGLA_NUCLEAR, REGLA_TONO, PILARES_11 };

...
PROHIBICIÓN: No te presentes en las secciones. No uses "Hola" ni frases de relleno. 
...
const PROMPTS = {
    INTRO: (dna) => `Genera la INTRODUCCIÓN CORPORATIVA de PredictaCore para ${dna}. Define la empresa como una unidad de ingeniería forense de conversión. Explica el uso de Gemelos Sintéticos y la metodología JTBD.`,
    DNA: (dna) => `Realiza el DIAGNÓSTICO DE INGENIERÍA de 5 PUNTOS para ${dna}. 1. Arquitectura de Navegación. 2. Transparencia Técnica (Activos omitidos). 3. Semiótica Visual. 4. Visión de Google (SEO local). 5. Percepción Algorítmica (Confianza inicial).`,
    GEMELOS: (dna) => `Define el PERFIL PSICOLÓGICO de 2 Gemelos Sintéticos para ${dna}: 1. ANA (Seguridad/Emoción) y 2. ROBERTO (Estatus/Eficiencia). Solo identidad y motivaciones psicológicas.`,
    ...
    VISIBILIDAD: (dna) => `AUDITORÍA DE VISIBILIDAD EXTERNA para ${dna}. Analiza SEO de intención, canibalización de plataformas externas y presencia local (Maps).`,

function getHTML() {
    ...
    <head>
        ...
        <style>
            ...
            .markdown-content ul, .markdown-content ol { margin-left: 1.5rem; margin-bottom: 1.5rem; list-style-type: decimal; }
    ...
    <body class="p-6 md:p-20">
        ...
        <script>
            ...
            // EL ESCUDO ANTI-MAYÚSCULAS: Detecta gritos y los convierte a formato elegante antes de imprimir
            function suavizarMayusculas(textoMarkdown) {
                // Separa por líneas para no afectar la estructura Markdown
                const lineas = textoMarkdown.split('\\n');
                const lineasSuavizadas = lineas.map(linea => {
                    // Si la línea es un encabezado o una fila de tabla, la dejamos casi intacta o evaluamos con cuidado
                    if (linea.startsWith('#') || linea.trim().startsWith('|')) {
                        return linea;
                    }
                    // Si más del 40% de las letras de la línea están en mayúscula, la suavizamos
                    const letras = linea.replace(/[^A-Za-zÁÉÍÓÚáéíóúÑñ]/g, '');
                    if (letras.length > 10) {
                        const mayusculas = linea.replace(/[^A-ZÁÉÍÓÚÑ]/g, '').length;
                        if (mayusculas / letras.length > 0.4) {
                            return linea.charAt(0).toUpperCase() + linea.slice(1).toLowerCase();
                        }
                    }
                    return linea;
                });
                return lineasSuavizadas.join('\\n');
            }
            ...
            async function ejecutar() {
                ...
                for (const etapa of etapas) {
                    ...
                    try {
                        ...
                        // Aplicamos el escudo anti-mayúsculas antes de procesar el Markdown
                        ...
                        // EL RESPIRADOR: Pausa de 3 segundos para asegurar la estabilidad de la conexión con Vertex AI
